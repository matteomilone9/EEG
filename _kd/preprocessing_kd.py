...
# Numero di canali GAF prodotti da make_gaf (= n_channels della baseline)
N_GAF_CHANNELS = KD_CFG["n_channels"]  # 22
...
def preprocess_subject(X_tr_raw: np.ndarray, X_te_raw: np.ndarray, cfg: dict):
    X_tr_t = zscore_per_trial(X_tr_raw)
    X_te_t = zscore_per_trial(X_te_raw)
    if cfg["use_gaf"]:
...
def split_train_val(Xt: np.ndarray, Xg: np.ndarray, y: np.ndarray, ratio: float = 0.8):
    ...
    idx_tr, idx_va = next(sss.split(Xt, y))
    return Xt[idx_tr], Xg[idx_tr], y[idx_tr], Xt[idx_va], Xg[idx_va], y[idx_va]
