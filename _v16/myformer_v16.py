...
# myformer_v16.py — TCFormer + ECA + S&R + Mixup + Multi-Seed
#                   + LOSO + Cross-Attention GAF Fusion
#                   + Fine-Tuning Post-LOSO
# ============================================================
# CAMBIAMENTI v15 → v16
#
# [CROSS_ATTENTION] cross_attention=False (default) — ripristinato AuxHead v14
#   La cross-attention (v15) non ha mostrato miglioramenti significativi
#   (+0.01% di media su 5 seed). Il flag rimane disponibile per ablation,
#   ma il default torna a False (AuxHead additiva).
#
# [FINE-TUNING POST-LOSO] Nuova pipeline LOSO → WS in due fasi:
#
#   FASE 1 — Pre-training LOSO (cross-subject):
#     Train = T+E di tutti i soggetti TRANNE il target → backbone generalizzato
#
#   FASE 2 — Fine-tuning Within-Subject (subject-specific):
#     Parte dai pesi LOSO (non da zero)
#     Train = sessione T del soggetto target SOLTANTO
#     lr ridotto (ft_lr=1e-4, 10× più basso del LOSO)
#     Epoche ridotte (ft_epochs=300), patience ridotta (ft_patience=100)
#     Freeze opzionale del backbone (ft_freeze_backbone):
#       True  → congela conv+transformer+tcn, aggiorna solo TCNHead+AuxHead
#       False → fine-tuning completo (tutti i layer, lr basso)
#
#   Razionale:
#   - Il LOSO fornisce un prior cross-subject (feature EEG generalizzabili)
#   - Il fine-tuning adatta il modello al pattern specifico del soggetto
#   - Cruciale per S02/S06 dove il WS puro fatica (segnale rumoroso)
#   - Standard in Transfer Learning / Domain Adaptation per EEG BCI
#
# Parametri nuovi in CFG:
#   'loso_ft_approach':   False  — True = LOSO+FT | False = WS o LOSO puro
#   'ft_lr':              1e-4   — lr fase 2
#   'ft_epochs':          300    — epoche max fase 2
#   'ft_patience':        100    — early stop fase 2
#   'ft_freeze_backbone': False  — True = congela backbone in fase 2
#
# Modifiche applicate (tutto il resto invariato da v15):
#   1. CFG: cross_attention=False, aggiunti ft_* e loso_ft_approach
#   2. _mode_tag: mostra modalità (WS / LOSO / LOSO+FT)
#   3. FineTuner: trainer leggero per fase 2
#   4. run_loso_ft_fold: pipeline completa LOSO→FT (singolo fold+seed)
#   5. run_loso_ft_fold_multiseed: wrapper multi-seed
#   6. Entry point: triforcato WS / LOSO / LOSO+FT
# ============================================================
...
CFG = {
    ...
    'loso_ft_approach':   True, # True = LOSO pre-train + WS fine-tuning
    'ft_lr':              1e-4,  # lr fase 2 (10× più basso del pre-train)
    'ft_epochs':          300,   # epoche max fine-tuning
    'ft_patience':        100,   # early stop fine-tuning
    'ft_freeze_backbone': False, # True = congela conv+transformer+tcn
    # ─────────────────────────────────────────────────────────
    # Multi-seed
    'multi_seed': False,
...
# ============================================================
# Preprocessing (invariato da v15)
# ============================================================
...
# ============================================================
# S&R, Mixup, Dataset, Utility (invariati da v15)
# ============================================================
...
class Conv1dWithConstraint(nn.Conv1d):
    def __init__(self, *args, max_norm=1.0, **kw):
        self.max_norm = max_norm; super().__init__(*args, **kw)
    def forward(self, x):
        self.weight.data = torch.renorm(self.weight.data, p=2, dim=0, maxnorm=self.max_norm)
        return super().forward(x)

# ============================================================
# ECA, ChannelGroupAttention, MultiKernelConvBlock
# ============================================================

class ECABlock2d(nn.Module):
    def __init__(self, channels):
        super().__init__()
        k = int(abs(math.log2(channels) + 1)); k = k if k % 2 == 1 else k + 1; k = max(k, 3)
    ...
    def __init__(self, n_channels, temp_kernel_lengths=(20,32,64),
        ...
        self.temporal_convs = nn.ModuleList([
            nn.Sequential(
                ...
                nn.Conv2d(1, F1, (1, k), bias=False), nn.BatchNorm2d(F1),
            ) for k in temp_kernel_lengths])
        n_groups = len(temp_kernel_lengths); self.d_model = d_group * n_groups; F2 = F1 * n_groups * D
        self.channel_DW_conv = nn.Sequential(
            nn.Conv2d(F1*n_groups, F2, (n_channels,1), bias=False, groups=F1*n_groups),
            nn.BatchNorm2d(F2), nn.ELU())
        self.pool1 = nn.AvgPool2d((1, pool_length_1)); self.drop1 = nn.Dropout(dropout)
        ...
        if self.use_group_attn: self.group_attn = ChannelGroupAttention(self.d_model, n_groups)
        self.pool2 = nn.AvgPool2d((1, pool_length_2)); self.drop2 = nn.Dropout(dropout)
        glorot_zero(self)
    def forward(self, x):
        x = self.rearrange(x)
        x = torch.cat([c(x) for c in self.temporal_convs], dim=1)
        x = self.drop1(self.pool1(self.channel_DW_conv(x))); x = self.eca(x)
        if self.use_cr2: x = self.channel_reduction_2(x)
        x = self.temporal_conv_2(x)
        if self.use_group_attn:
            xs = x.squeeze(2); xs = xs + self.group_attn(xs); x = xs.unsqueeze(2)
        return self.drop2(self.pool2(x)).squeeze(2)

# ============================================================
# RoPE + GQA + DropPath + TransformerBlock + TCN
# ============================================================
...
class GQAttention(nn.Module):
    def __init__(self, d_model, q_heads, kv_heads, dropout=0.3):
        ...
        self.qh, self.kvh = q_heads, kv_heads; self.hd = d_model // q_heads; self.scale = self.hd ** -0.5
        self.q_proj  = nn.Linear(d_model, d_model, bias=False)
        self.kv_proj = nn.Linear(d_model, 2*kv_heads*self.hd, bias=False)
        self.o_proj  = nn.Linear(d_model, d_model, bias=False)
        self.drop    = nn.Dropout(dropout); glorot_zero(self)
...
class DropPath(nn.Module):
    ...
    def forward(self, x):
        ...
        kp = 1 - self.p; shape = (x.shape[0],) + (1,)*(x.ndim-1)
...
class TransformerBlock(nn.Module):
    def __init__(self, d_model, q_heads, kv_heads, dropout=0.4, drop_path=0.0):
        super().__init__()
        self.norm1 = nn.LayerNorm(d_model); self.attn = GQAttention(d_model, q_heads, kv_heads, dropout)
        self.dp    = DropPath(drop_path);   self.norm2 = nn.LayerNorm(d_model)
    ...
    def forward(self, x, cos, sin):
        ...
        x = x + self.dp(self.mlp(self.norm2(x))); return x

class TCNBlock(nn.Module):
    def __init__(self, nf, ks=4, dil=1, ng=1, dropout=0.3):
        super().__init__()
        self.c1 = CausalConv1d(nf, nf, ks, dil, groups=ng); self.b1 = nn.BatchNorm1d(nf)
        self.e1 = nn.ELU(); self.d1 = nn.Dropout(dropout)
        self.c2 = CausalConv1d(nf, nf, ks, dil, groups=ng); self.b2 = nn.BatchNorm1d(nf)
        self.e2 = nn.ELU(); self.d2 = nn.Dropout(dropout); self.act = nn.ELU()
        nn.init.constant_(self.c1.bias, 0); nn.init.constant_(self.c2.bias, 0)
    def forward(self, x):
        h = self.d1(self.e1(self.b1(self.c1(x)))); h = self.d2(self.e2(self.b2(self.c2(h))))
        return self.act(x + h)
...
class TCNHead(nn.Module):
    ...
    def forward(self, x):
        x = self.tcn(x)[..., -1:]; x = self.cls(x).squeeze(-1)
...
class TCFormerModule(nn.Module):
    def __init__(self, n_channels, n_classes, F1=16, temp_kernel_lengths=(20,32,64),
        ...
        super().__init__()
        from einops.layers.torch import Rearrange
        n_groups = len(temp_kernel_lengths); self.d_model = d_group*n_groups
        self.d_group = d_group; self.n_groups = n_groups
        ...
        self.mix = nn.Sequential(nn.Conv1d(self.d_model, self.d_model, 1, bias=False),
        ...
        self.reduce = nn.Sequential(Rearrange("b t c -> b c t"),
    ...
    def get_features(self, x):
        conv_f = self.conv_block(x); B, C, T = conv_f.shape
        tok = self.to_seq(self.mix(conv_f)); cos, sin = self._rope_cache(T, x.device)
        for blk in self.transformer: tok = blk(tok, cos, sin)
        return torch.cat((conv_f, self.reduce(tok)), dim=1)
    def forward(self, x): return self.tcn_head(self.get_features(x))

# ============================================================
# GAF MiniEncoder + AuxHead + GAFCrossAttnHead
# ============================================================
...
class GAFMiniEncoder(nn.Module):
    def __init__(self, n_channels=22, out_dim=64, dropout=0.4):
        ...
        self.pool_ch = nn.Sequential(nn.Linear(out_dim*4, out_dim), nn.ELU(), nn.Dropout(dropout))
        glorot_zero(self)
    def forward(self, x):
        B, C, H, W = x.shape
...
class GAFCrossAttnHead(nn.Module):
    ...
    def __init__(self, d_tcf, gaf_dim, n_classes, dk=32, hidden=64, dropout=0.4, attn_dropout=0.3):
        super().__init__()
        self.scale     = dk ** -0.5
        ...
        self.cls = nn.Sequential(nn.Linear(d_tcf, hidden), nn.LayerNorm(hidden),
...
class TCFormerWithAux(nn.Module):
    ...
    def forward(self, eeg, gaf=None):
        if self.use_gaf and gaf is not None:
            # [FIX v16] Skip GAF encoder se il gaf è dummy (H==1, W==1)
            # Succede nella Fase 1 LOSO dove non ci sono GAF reali
            ...
            if gaf_is_real:
                ...
                # Fase 1 LOSO: niente aux loss, ritorna (logits_main, None)
...
# ============================================================
# DistillationTrainer (invariato da v15)
# ============================================================

class DistillationTrainer:
    def __init__(self, model, cfg):
        self.model     = model.to(cfg['device']); self.device = cfg['device']; self.cfg = cfg
        self.use_gaf   = cfg['use_gaf'];           self.use_mixup = cfg['use_mixup']
        self.ce        = nn.CrossEntropyLoss()
        self.opt       = torch.optim.Adam(model.parameters(), lr=cfg['lr'])
        self.sched     = torch.optim.lr_scheduler.CosineAnnealingLR(
            self.opt, T_max=cfg['epochs'], eta_min=1e-5)
        self.best_acc  = 0.0; self.best_state = None
    ...
    def _epoch(self, loader, train):
        self.model.train(train); tot_loss, preds, trues = 0.0, [], []
        lam = self._lambda(); use_gaf = self.use_gaf; use_mixup = self.use_mixup
        with torch.set_grad_enabled(train):
            for b in loader:
                eeg = b['eeg'].to(self.device); gaf = b['gaf'].to(self.device); y = b['label'].to(self.device)
                if train:
                    eeg = segment_and_reconstruct(eeg, y, n_segments=self.cfg['n_segments'], sr_prob=self.cfg['sr_prob'])
                    self.opt.zero_grad()
                    if use_mixup:
                        eeg, y_soft = mixup_batch(eeg, y, n_classes=self.cfg['n_classes'],
                                                  mixup_prob=self.cfg['mixup_prob'], alpha=self.cfg['mixup_alpha'])
                        if use_gaf:
                        ...
                        if use_gaf:
                            ...
                            logits_m = self.model(eeg); loss = self.ce(logits_m, y)
                    loss.backward(); self.opt.step()
                else:
                    logits_m = self.model(eeg, gaf=None) if use_gaf else self.model(eeg)
                    loss = self.ce(logits_m, y)
                tot_loss += loss.item()
                preds.extend(logits_m.argmax(-1).cpu().numpy()); trues.extend(y.cpu().numpy())
        acc = accuracy_score(trues, preds)*100; kappa = cohen_kappa_score(trues, preds)
    ...
    def fit(self, tr_ld, va_ld, seed=42):
        ...
        print(f"\n{'='*65}\nMode: {_mode_tag(self.cfg)} | seed={seed} | epochs={epochs} | patience={pat}\n{'='*65}")
        for ep in range(epochs):
            tr_loss, tr_acc, _    = self._epoch(tr_ld, True)
            va_loss, va_acc, va_k = self._epoch(va_ld, False)
            self.sched.step(); lr = self.opt.param_groups[0]['lr']
            for k, v in zip(['tr_loss','va_loss','tr_acc','va_acc','va_kap'],
                            [tr_loss, va_loss, tr_acc, va_acc, va_k]):
                self.history[k].append(v)
            if va_acc > self.best_acc:
                self.best_acc = va_acc; self.best_state = copy.deepcopy(self.model.state_dict())
                wait = 0; tag = ' ✨ BEST'
            else:
                wait += 1; tag = f' [{wait}/{pat}]'
            lam_str = f"λ={self._lambda():.3f} | " if self.use_gaf else ""
            print(f"Ep {ep+1:03d}/{epochs} | {lam_str}LR {lr:.1e} | Tr {tr_acc:.1f}% {tr_loss:.4f} | Va {va_acc:.1f}% {va_loss:.4f} k={va_k:.3f}{tag}")
            if wait >= pat: print(f"\n🛑 Early stop ep {ep+1}"); break
        if self.best_state: self.model.load_state_dict(self.best_state)
        print(f"\n🏆 Best val: {self.best_acc:.2f}%"); return self.history

# ============================================================
# [v16] FineTuner — Fase 2: fine-tuning subject-specific
# ============================================================

class FineTuner:
    """
    Trainer per la Fase 2 del fine-tuning post-LOSO.
    Parte dai pesi di un modello pre-addestrato (LOSO).
    Usa lr ridotto, meno epoche, opzionalmente congela il backbone.

    Freeze opzionale (ft_freeze_backbone=True):
      - Congela: conv_block, mix, transformer, reduce (backbone TCFormer)
      - Libera:  tcn_head, gaf_enc, aux_head (classificatori finali)
      - Rationale: il backbone ha già imparato feature cross-subject,
        il fine-tuning adatta solo i layer di decisione al target
    """
    def __init__(self, model, cfg):
        ...
        if cfg.get('ft_freeze_backbone', False):
            # Congela tutto il backbone TCFormer
            for name, param in model.tcformer.named_parameters():
                if any(part in name for part in ['conv_block', 'mix', 'transformer', 'reduce']):
                ...
                if train:
                    ...
                    if use_mixup:
                        ...
                        if use_gaf:
                            ...
                            loss = self.ce(logits_m, y)
                            if logits_a is not None:
                                loss = loss + lam*self.ce(logits_a, y)
                        else:
                            logits_m = self.model(eeg); loss = self.ce(logits_m, y)
                    loss.backward(); self.opt.step()
                else:
    ...
    def fit(self, tr_ld, va_ld, seed=42):
        epochs = self.cfg['ft_epochs']; pat = self.cfg['ft_patience']; wait = 0
        ...
        for ep in range(epochs):
            ...
            if va_acc > self.best_acc:
                ...
                wait += 1; tag = f' [{wait}/{pat}]'
            print(f"  FT {ep+1:03d}/{epochs} | LR {lr:.1e} | Tr {tr_acc:.1f}% {tr_loss:.4f} | Va {va_acc:.1f}% {va_loss:.4f} k={va_k:.3f}{tag}")
            if wait >= pat: print(f"\n  🛑 FT early stop ep {ep+1}"); break
        if self.best_state: self.model.load_state_dict(self.best_state)
        print(f"\n  🏆 FT Best val: {self.best_acc:.2f}%"); return self.best_acc

# ============================================================
# evaluate (invariato da v15)
# ============================================================
...
def evaluate(model, loader, device, cfg):
    model.eval(); n_tta = cfg['n_tta']; use_gaf = cfg['use_gaf']
    use_gaf_inf = use_gaf and cfg['gaf_inference']; alpha = cfg['gaf_inference_alpha']
    preds, trues = [], []
    with torch.no_grad():
        for b in loader:
            eeg = b['eeg'].to(device); gaf = b['gaf'].to(device); y = b['label'].numpy()
            if use_gaf_inf:
                feat = model.tcformer.get_features(eeg); gaf_emb = model.gaf_enc(gaf)
                ...
                for _ in range(n_tta-1): logits = logits + model(_tta_augment(eeg), gaf=None)
                logits /= max(n_tta, 1)
            else:
                logits = model(eeg)
                for _ in range(n_tta-1): logits = logits + model(_tta_augment(eeg))
                logits /= max(n_tta, 1)
            preds.extend(logits.argmax(-1).cpu().numpy()); trues.extend(y)
    return np.array(trues), np.array(preds)

# ============================================================
# Within-Subject: run_subject / run_subject_multiseed
# ============================================================
...
def run_subject(sub_id, seed=42, verbose=True):
    ...
    set_seed(seed); model = build_model(cfg); trainer = DistillationTrainer(model, cfg)
    trainer.fit(tr_ld, te_ld, seed=seed)
    y_true, y_pred = evaluate(trainer.model, te_ld, cfg['device'], cfg)
    acc = accuracy_score(y_true, y_pred)*100; kappa = cohen_kappa_score(y_true, y_pred)
    if verbose:
...
def run_subject_multiseed(sub_id):
    cfg = get_subject_cfg(sub_id); seeds = cfg['seeds']
    ...
    for i, seed in enumerate(seeds):
        print(f"\n  ── Seed {i+1}/{len(seeds)}: {seed} ──"); set_seed(seed)
        model = build_model(cfg); trainer = DistillationTrainer(model, cfg)
        trainer.fit(tr_ld, te_ld, seed=seed)
        y_true, y_pred = evaluate(trainer.model, te_ld, cfg['device'], cfg)
        acc = accuracy_score(y_true, y_pred)*100; kappa = cohen_kappa_score(y_true, y_pred)
...
# ============================================================
# LOSO puro: run_loso_fold / run_loso_fold_multiseed
# ============================================================
...
def run_loso_fold(target_sub, cache, seed=42, verbose=True):
    ...
    tr_ds = MMDataset(X_tr, _make_dummy_gaf(len(X_tr)), y_tr, augment=True,  aug_prob=cfg['aug_prob'])
    te_ds = MMDataset(X_te, _make_dummy_gaf(len(X_te)), y_te, augment=False, aug_prob=0.0)
    tr_ld = DataLoader(tr_ds, cfg['batch_size'], shuffle=True,  num_workers=0)
    te_ld = DataLoader(te_ds, cfg['batch_size'], shuffle=False, num_workers=0)
    set_seed(seed); model = build_model(cfg); trainer = DistillationTrainer(model, cfg)
    trainer.fit(tr_ld, te_ld, seed=seed)
...
# ============================================================
# [v16] LOSO + Fine-Tuning: run_loso_ft_fold / multiseed
# ============================================================

def run_loso_ft_fold(target_sub, cache, seed=42, verbose=True):
    """
    Pipeline completa LOSO → Fine-Tuning per un singolo fold.

    FASE 1 — Pre-training LOSO:
      Train = T+E di tutti i soggetti TRANNE target_sub
      Produce backbone con conoscenza cross-subject

    FASE 2 — Fine-tuning Within-Subject:
      Parte dai pesi LOSO (non da zero)
      Train = sessione T del soggetto target
      Val/Test = sessione E del soggetto target
      lr ridotto (ft_lr), freeze opzionale backbone

    I dati del soggetto target vengono caricati freschi (con GAF se use_gaf=True)
    perché nella cache LOSO i GAF non sono stati calcolati per il target.
    """
    cfg = get_subject_cfg(target_sub)
    if verbose:
        print(f"\n{'─'*55}")
        print(f"[LOSO+FT] S{target_sub:02d} | seed={seed}")
        print(f"{'─'*55}")
    ...
    # Test set provvisorio (sessione E del target — già in cache, zscorato)
    X_te, y_te = cache[target_sub]['E']

    loso_tr_ds = MMDataset(X_tr_loso, _make_dummy_gaf(len(X_tr_loso)),
                           y_tr_loso, augment=True, aug_prob=cfg['aug_prob'])
    loso_te_ds = MMDataset(X_te, _make_dummy_gaf(len(X_te)), y_te, augment=False, aug_prob=0.0)
    ...
    # Carica dati subject-specific con GAF (se use_gaf=True)
    ...
    # Reabilita tutti i requires_grad (nel caso siano stati congelati da un run precedente)
    for p in model.parameters(): p.requires_grad = True

    fine_tuner = FineTuner(model, cfg)
    fine_tuner.fit(ft_tr_ld, ft_te_ld, seed=seed)

    # Valutazione finale (con TTA)
...
def run_loso_ft_fold_multiseed(target_sub, cache):
    cfg = get_subject_cfg(target_sub); seeds = cfg['seeds']
    print(f"\n{'═'*65}")
    print(f"[LOSO+FT] S{target_sub:02d} | Multi-seed ({len(seeds)} run): {seeds}")
    print(f"{'═'*65}")
...
# ============================================================
# Entry point — triforcato: WS / LOSO / LOSO+FT
# ============================================================
    ...
    if CFG.get('loso_ft_approach', False):
        ...
        if CFG['multi_seed']:
            for s in range(1, CFG['n_subjects'] + 1):
                ...
                results[s] = {'acc': mean_acc, 'std': std_acc,
                              'kappa': mean_k, 'kappa_std': std_k,
                              'seeds': seed_accs, 'loso_acc': mean_loso}
        else:
            for s in range(1, CFG['n_subjects'] + 1):
                acc, kappa, acc_loso = run_loso_ft_fold(s, cache, seed=42)
                results[s] = {'acc': acc, 'std': 0.0, 'kappa': kappa,
                              'kappa_std': 0.0, 'seeds': [acc], 'loso_acc': acc_loso}
        ...
        if CFG['multi_seed']:
            for s in range(1, CFG['n_subjects'] + 1):
                ...
                results[s] = {'acc': mean_acc, 'std': std_acc, 'kappa': mean_k,
                              'kappa_std': std_k, 'seeds': seed_accs, 'loso_acc': mean_acc}
        else:
            for s in range(1, CFG['n_subjects'] + 1):
                acc, kappa = run_loso_fold(s, cache, seed=42)
                results[s] = {'acc': acc, 'std': 0.0, 'kappa': kappa,
                              'kappa_std': 0.0, 'seeds': [acc], 'loso_acc': acc}
        ...
        if CFG['multi_seed']:
            for s in range(1, CFG['n_subjects'] + 1):
                ...
                results[s] = {'acc': mean_acc, 'std': std_acc, 'kappa': mean_k,
                              'kappa_std': std_k, 'seeds': seed_accs, 'loso_acc': None}
        else:
            for s in range(1, CFG['n_subjects'] + 1):
                acc, kappa = run_subject(s, seed=42)
                results[s] = {'acc': acc, 'std': 0.0, 'kappa': kappa,
                              'kappa_std': 0.0, 'seeds': [acc], 'loso_acc': None}
    ...
    if CFG['multi_seed']:
        ...
        for s in range(1, 10):
            ...
            if show_loso_col:
                ...
                delta = results[s]['acc'] - la if la is not None else 0.0
                row += f" | {la:8.2f} | {delta:+7.2f}"
            print(row)
        print('-' * (len(hdr)))
        ...
        if show_loso_col:
            loso_accs = [results[s]['loso_acc'] for s in range(1,10) if results[s]['loso_acc'] is not None]
