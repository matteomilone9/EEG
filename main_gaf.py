...
def main():
    ...
    print(f"🖼️  GAF type    : {gaf_type} | image_size: {image_size}x{image_size}")
    ...
    train_dataset = EEGGAFDataset(df, labels, train_range, window_size, stride=stride,
                                  gaf_type=gaf_type, image_size=image_size, augment=True)
    val_dataset   = EEGGAFDataset(df, labels, val_range,   window_size, stride=stride,
                                  gaf_type=gaf_type, image_size=image_size, augment=False)
    test_dataset  = EEGGAFDataset(df, labels, test_range,  window_size, stride=stride,
                                  gaf_type=gaf_type, image_size=image_size, augment=False)
    ...
    # ── 6. Checkpoint dir ────────────────────
    ckpt_dir    = "./results/checkpoints/GAFNET"
